...
### POST /deposits
Открыть срочный вклад. Принимает JSON `{customer_id, amount_rub, term_months}`.
Списывает `amount_rub` с баланса клиента и сохраняет запись о вкладе.
Ставка назначается автоматически по сроку: 3 мес → 7%, 6 → 9.5%, 12 → 12%, 24 → 11%, 36+ → 10.5%.
Возвращает `{deposit_id, customer_id, amount_rub, term_months, rate_pct, maturity_date, new_balance_rub}`.
Ошибка `404` если клиент не найден, `400` если недостаточно средств.
