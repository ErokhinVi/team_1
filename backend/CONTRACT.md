...
Seeded corporate ids имеют формат `corp-001`, `corp-002`, `corp-003`.
Персональные ids вида `c-01394` не являются корпоративными счетами.
...
Валидный пример: `GET /corporate/accounts/corp-001`.
...
### POST /deposits
Открыть срочный вклад. Принимает JSON `{customer_id, amount_rub, term_months, rate_pct?}`.
Списывает `amount_rub` с баланса клиента и сохраняет запись о вкладе.
`rate_pct` — опционально: если cib передаёт персональную ставку, она используется как есть;
если не передана — ставка назначается по сроку: 3 мес → 7%, 6 → 9.5%, 12 → 12%, 24 → 11%, 36+ → 10.5%.
Возвращает `{deposit_id, customer_id, amount_rub, term_months, rate_pct, maturity_date, new_balance_rub}`.
Ошибка `404` если клиент не найден, `400` если недостаточно средств или ставка ≤ 0.

### POST /loans
Выдать потребительский кредит клиенту. Принимает JSON `{customer_id, amount_rub, term_months, rate_pct}`.
Зачисляет `amount_rub` на счёт клиента и сохраняет запись о кредите.
