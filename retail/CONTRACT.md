...
Проверка права на реферальный бонус (прокси к cib `POST /referral/validate`).
Принимает `{referrer_id}`. Возвращает `{eligible, reason}`.
Отказ, если пригласивший не клиент банка или достиг лимита приглашений (10).

### POST /api/referrals
Оформление приглашения друга (прокси к backend `POST /referrals`).
Принимает `{referrer_id, new_customer_name}`. Открывает счёт другу и начисляет
бонус 20 000 ₽ обоим. Возвращает `{referral_id, referrer_id, new_customer_id,
new_customer_name, bonus_rub, referrer_new_balance_rub, new_customer_balance_rub}`.
Сначала вызывается `/api/referral/validate` (cib), затем backend.
...
### GET /bonds
Перенаправляет на `/brokerage` — облигации теперь живут на странице «Инвестиции»
вместе с акциями (под-вкладка «Облигации»). Оставлено для совместимости со старыми ссылками.

### GET /api/bonds/catalogue
Каталог облигаций (прокси к cib `GET /products/bonds`).
Возвращает `{total, items: [{bond_id, issuer, name, kind, coupon_pct, maturity_date, face_value_rub, price_rub}]}`.

### GET /api/bonds/recommendation/{customer_id}
Рекомендация по облигациям (прокси к cib). Возвращает `{customer_id, risk_score, recommendation: [{bond_id, allocation_pct}], note}`.

### GET /api/bonds/holdings/{customer_id}
Облигации клиента (прокси к backend). Возвращает `{customer_id, total, items: [{bond_id, quantity, avg_price_rub}]}`.

### POST /api/bonds/orders
Покупка/продажа облигаций (прокси к backend). Принимает `{customer_id, bond_id, quantity, direction}` (`direction` = `buy`|`sell`).
Возвращает `{order_id, status, bond_id, direction, quantity, total_rub, new_balance_rub}`.

### GET /brokerage
HTML-страница «Инвестиции» — акции и облигации в одном экране (две под-вкладки). Показывает список доступных акций, баланс брокерского счёта клиента, персональную рекомендацию по портфелю и форму для покупки/продажи. Для человека, не для других блоков.
...
- cib: `POST /credit-decision`; `GET /products/brokerage`; `GET /brokerage/recommendation/{id}`; `POST /corporate/payment-auth`; `POST /payroll/validate`; `POST /loan/decision`; `POST /mortgage/decision`; `POST /deposit/terms`; `GET /products/bonds`; `GET /bonds/recommendation/{id}`
- backend (ипотека/облигации): `POST /mortgages`, `GET /bonds/holdings/{id}`, `POST /bonds/orders`
